...
(function () {
  ...
  window.KaushoraAPI = {
    ...
    districts: () => cached("dist", () => get("/api/districts")),
    district: (id) => get("/api/districts/" + encodeURIComponent(id)),
    districtSkills: (id) => get("/api/districts/" + encodeURIComponent(id) + "/skills"),
    indicators: (stateId) => cached("ind" + (stateId || "all"), () => get("/api/indicators" + (stateId ? "?state_id=" + encodeURIComponent(stateId) : "")).then(unwrap)),
    evidence: () => cached("ev", () => get("/api/evidence").then(unwrap)),
    recommendations: () => cached("rec", () => get("/api/recommendations").then(unwrap)),
    ...
    jobs: (params) => get("/api/jobs" + (params || "")).then(unwrap),
